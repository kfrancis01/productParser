const goodIngredients = {
    ...
    betaine:
      "Betaine: A humectant and conditioning agent that can help improve hair hydration.",
    ...
    "polyglyceryl-3 betainate acetate":
      "Polyglyceryl-3 Betainate Acetate: A conditioning agent for hair.",
    "kalahari melon seed oil":
      "Kalahari Melon Seed Oil: Intensely hydrates to promote hair growth and target hair loss.",
    "red raspberry seed oil":
      "Red Raspberry Seed Oil: Nourishes dry scalps and prevents moisture loss for hydrated, shiny hair.",
    "strawberry seed oil":
      "Strawberry Seed Oil: Encourages a healthy scalp environment and smooths hair to reduce frizz.",
    // Add more key-value pairs for other ingredients
  };
  ...
  function highlightIngredientsInSections(textToMatch, borderColor) {
    const elements = document.querySelectorAll("*"); // Select all elements in the page
    //const elements = document.querySelectorAll("h2, div, span");
    let insideSection = false;
  
    // define the headers for the list of information you want to parse
  ...
  for (const [key, value] of Object.entries(goodIngredients)) {
    highlightIngredientsInSections(key, "green");
  }
  for (const [key, value] of Object.entries(badIngredients)) {
    highlightIngredientsInSections(key, "red");
  }
  for (const [key, value] of Object.entries(midIngredients)) {
    highlightIngredientsInSections(key, "yellow");
  }
  
  // Function to create and display a popup with a border
  function createPopupWithBorder(title, message, backgroundColor, borderColor) {
    const popup = document.createElement('*');
    popup.style.backgroundColor = backgroundColor;
    popup.style.color = 'white';
    popup.style.position = 'fixed';
    popup.style.top = '0px';
    popup.style.right = '0px'; // Position to the right
    ...
    closeButton.style.display = 'block'; // Place the close button on its own row
    closeButton.style.marginTop = '5px'; // Add some top margin for spacing
    ...
    popup.appendChild(document.createTextNode(title)).style.display = 'block';
  ...
  for (const [key, value] of Object.entries(badIngredients)) {
    if (document.body.textContent.includes(key)) {
      badIngredientsFound.push(`${value}`);
      highlightIngredientsInSections(key, "red");
    }
  }
  ...
  for (const [key, value] of Object.entries(midIngredients)) {
    if (document.body.textContent.includes(key)) {
      midIngredientsFound.push(`${value}`);
      highlightIngredientsInSections(key, "yellow");
    }
  }
  
  if (midIngredientsFound.length > 0) {
    // Create a popup with a list of bad ingredients found
    createPopupWithBorder(
  ...
  for (const [key, value] of Object.entries(goodIngredients)) {
    if (document.body.textContent.includes(key)) {
      goodIngredientsFound.push(`${value}`);
      highlightIngredientsInSections(key, "green");
    }
  }
  
  if (goodIngredientsFound.length > 0) {
    // Create a popup with a list of good ingredients found
    createPopupWithBorder(
      "\nGreat Choice:\n", 
